...
class UserList extends Component
{
    ...
    public function mount(){
        ...
        if (Auth::user()->hasRole('DSI God Admin')) {
            // Show all roles if the user is a DSI God Admin
            $this->roles = Role::all();
        } else {
            // Exclude the "DSI God Admin" role for other users
            $this->roles = Role::where('name', '!=', 'DSI God Admin')->get();
        }
    }
    ...
    public function render()
    {
        ...
        if (!empty($this->search)) {
            ...
            $users = $users->where(function($query) use ($search){
                $query =  $query->where('users.name','LIKE','%'.$search.'%')
                    ...
                    ->orWhere('users.guardian_email','LIKE','%'.$search.'%');
            });


        }
        if (!empty($this->selected_gender)) {
            $users = $users->where('users.gender',$this->selected_gender);
        }
        ...
        /*
            ...
            if (!Auth::user()->hasRole('DSI God Admin') && !Auth::user()->hasRole('Admin')) {
                ...
                $users = $users->whereNotIn('users.created_by', $dsiGodAdminUserIds);
            } else {
        ...
        */
        ...
        // 🚀 **Exclude "DSI God Admin" users if the logged-in user does not have that role**
        if (!Auth::user()->hasRole('DSI God Admin')) {
            $users = $users->whereDoesntHave('roles', function ($query) {
                $query->where('name', 'DSI God Admin');
            });
        }
