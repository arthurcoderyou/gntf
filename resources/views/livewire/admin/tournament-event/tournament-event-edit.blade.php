        ...
        <div class="bg-white rounded-xl shadow dark:bg-neutral-900">
            ...
            <div class="  p-4">
                ...
                <div class="sm:col-span-12">
                    ...
                    @if(!empty($tournament))
                        for <span class="text-gntf_green-500">{{ $tournament->name }}</span> 
                    @endif
                    </h2>
                    @if(!empty($tournament))
                        <span class="text-gray-500 block text-sm">
                            ...
                            @if(!empty($tournament->end_date))
                                to {{ \Carbon\Carbon::parse($tournament->end_date)->format('M d, Y') }}
                            @endif
                        </span> 
                ...
                <div class="grid grid-cols-12 gap-x-2  ">
                    ...
                    <div class="space-y-2 col-span-12  sm:col-span-4 ">
                        ...
                        <div
                            ...
                            <div class="flex items-center  justify-between">
                                ...
                                @if(!empty($tournament))
                                    <span class="text-gntf_green-500">{{ $tournament->name }}</span> 
                                @else  
                                    Search Tournament
                    ...
                    <div class="space-y-2 col-span-12  sm:col-span-4 ">
                        <label for="junior_event" class="inline-block text-sm font-medium text-gray-800 mt-2.5 dark:text-neutral-200">
                            Junior Event?
                        </label> 
                        <select 
                        wire:model.live="junior_event"
                        id="junior_event" name="junior_event"
                    ...
                    <div class="space-y-2 col-span-12 sm:col-span-4"> 
                        <label for="registration_deadline" class="inline-block text-sm font-medium text-gray-800 mt-2.5 dark:text-neutral-200">
                            Registration Deadline
                        </label>
                        <input type="text" wire:model.live='date' id="registration_deadline" class="py-2 px-3 pe-11 block w-full border-gray-200 shadow-sm rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" readonly
...
<script>
    $(document).ready(function() {
        ...
        // Initialize Flatpickr for a single date picker
        flatpickr("#registration_deadline", {
            dateFormat: "M d, Y", // Format as `Month Day, Year`
            onChange: function(selectedDates, dateStr) {
                if (selectedDates.length === 1) {
                    const selectedDate = flatpickr.formatDate(selectedDates[0], "Y-m-d");
                    // Send the selected date to Livewire
                    @this.set('registration_deadline', selectedDate);
                }
            }
        });
