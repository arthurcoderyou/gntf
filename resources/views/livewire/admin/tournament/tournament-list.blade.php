        ...
        <div class="p-1.5 min-w-full inline-block align-middle">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                    ...
                    @if(!empty($tournaments) && count($tournaments) > 0)
                        @foreach ($tournaments as $tournament)
                            <tr>
                                <td class="w-4 whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        <label for="data_{{ $tournament->id }}" class="flex">
                                            <input type="checkbox"
                                            wire:model="selected_records"
                                            wire:change="updateSelectedCount"
                                            class="shrink-0 border-gray-300 rounded text-blue-600 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-600 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800"
                                            id="data_{{ $tournament->id }}"
                                            value="{{ $tournament->id }}"
                                            >
                                            <span class="sr-only">Checkbox</span>
                                        </label>
                                ...
                                <td class="size-auto whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        @if(!empty($tournament->logo))
                                            <img src="{{ asset('storage/uploads/tournament/'.$tournament->logo) }}" class="max-w-48" alt="">
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="size-auto whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        ...
                                        <div class="grow">
                                            <span class="block text-sm text-gray-500 dark:text-neutral-500">{{ $tournament->name }}</span>
                                        </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="size-auto whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        <div class="flex items-center gap-x-3">
                                        <div class="grow">
                                            <span class="block text-sm text-gray-500 dark:text-neutral-500">{{ $tournament->director }}</span>
                                        </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="size-auto whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        <div class="flex items-center gap-x-3">
                                        <div class="grow">
                                            <span class="block text-sm text-gray-500 dark:text-neutral-500">{{ $tournament->finance }}</span>
                                        </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="size-auto whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        ...
                                        <div class="grow">
                                            ...
                                            <span class="block text-sm text-gray-500 dark:text-neutral-500">
                                                Updated at 
                                                <br> <strong>{{ \Carbon\Carbon::parse($tournament->updated_at)->format('M d, Y') }}</strong> 
                                                <br> <strong>{{ \Carbon\Carbon::parse($tournament->updated_at)->format('h:i A') }}</strong>
                                                <br>Created by 
                                                <br> <strong>{{ $tournament->creator ? $tournament->creator->name : '' }}</strong>
                                ...
                                <td class="w-4 whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        ...
                                        <!-- events -->
                                        <a href="{{ route('tournament_event.index',['tournament_id' => $tournament->id]) }}" class="py-2 px-3 inline-flex items-center gap-x-2  text-sm font-medium rounded-lg border border-transparent bg-yellow-600 text-white hover:bg-yellow-700 focus:outline-none focus:bg-yellow-700 disabled:opacity-50 disabled:pointer-events-none">
                                             
                                            <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill="#ffffff" d="M96 32l0 32L48 64C21.5 64 0 85.5 0 112l0 48 448 0 0-48c0-26.5-21.5-48-48-48l-48 0 0-32c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 32L160 64l0-32c0-17.7-14.3-32-32-32S96 14.3 96 32zM448 192L0 192 0 464c0 26.5 21.5 48 48 48l352 0c26.5 0 48-21.5 48-48l0-272z"/></svg>
                                        </a>

                                        @if( Auth::user()->can('tournament edit') ||  Auth::user()->hasRole('DSI God Admin'))
                                            <!-- edit -->
                                            <a href="{{ route('tournament.edit',['tournament' => $tournament->id]) }}" class="py-2 px-3 inline-flex items-center gap-x-2  text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
                                        ...
                                        @if( Auth::user()->can('tournament delete') ||  Auth::user()->hasRole('DSI God Admin'))
                                            ...
                                            wire:click.prevent="delete({{ $tournament->id }})"
            ...
            <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200 dark:border-neutral-700">
                ...
                <div class="inline-flex items-center gap-x-2">
                    ...
                    <p class="text-sm text-gray-600 dark:text-neutral-400">
                        {{ count($tournaments) > 0 ? 'of '.$tournaments->total()  : '' }}
                    </p>
                </div>
